...
class StateAddress extends State<ManageAddress> with TickerProviderStateMixin {
    ...
    try {
      ...
      Response response =
          await post(updateAddressApi, body: data, headers: headers)
              .timeout(Duration(seconds: timeOut));
      print(updateAddressApi.toString());
      print(data.toString());
  ...
  void addAddressModel() {
    for (int i = 0; i < addressList.length; i++) {
      print("address items are here ${addressList[i].address} and ${addressList[i].area} and ${addressList[i].city} and ${addressList[i].state} and ${addressList[i].country} and ${addressList[i].pincode}");
      addModel.add(
          RadioModel(
          isSelected: i == selectedAddress ? true : false,
          name: "${addressList[i].name!}, ${addressList[i].mobile!}",
          add: "${addressList[i].address!}, ${addressList[i].city.toString()}, ${addressList[i].state!}, ${addressList[i].country!}, ${addressList[i].pincode!}",
