...
import 'package:eshop_multivendor/Helper/Public%20Api/api.dart';
import 'package:eshop_multivendor/Helper/Session.dart';
import 'package:eshop_multivendor/Helper/phone_gateway_helper.dart';
...
//List<SectionModel> cartList = [];
List<Promo> promoList = [];
double totalPrice = 0, oriPrice = 0, delCharge = 0, taxPer = 0,  tempTotaleAmt = 0;
int? selectedAddress = 0;
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
                ...
                children: [
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              ...
                              cartList[index].productList![0].availability == "1" ||
                                  cartList[index].productList![0].stockType == "0"
                                  ? Row(
                                children: <Widget>[
                                  Row(
                                    children: <Widget>[
                  ...
                  Column(
                      ...
                      cartList[index].addOns!.map((AddOnsModel e) {
                        ...
                        print("addd on imageeee ${e.image}");
                        return   Padding(
                          padding: const EdgeInsets.all(5.0),
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
  ...
  doPayment() {
    ...
    /*  PhonepeGatewayHelper phonepeGatewayHelper = PhonepeGatewayHelper(amount: totalPrice.toString(), context1: context, onResult: (result){

      });
      phonepeGatewayHelper.init();*/
      getPhonpayURL();
    }
      //getPhonpayURL();
    else if (payMethod == getTranslated(context, 'FLUTTERWAVE_LBL'))
  ...
  razorpayPayment() async {
    ...
    if (contact != '') {
      ...
      print("amonutt rajorpayyyyy ${amt}");
      try {
        _razorpay!.open(options);
      } catch (e) {
        debugPrint(e.toString());
    ...
    if (_isNetworkAvail) {
      ...
      try {
        var parameter = {
          ...
          WALLET_BAL_USED: usedBal.toString(),
          ORDER_NOTE: noteC.text,
          "receiver_address":receiverId.toString(),
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            String orderId = getdata["order_id"].toString();
            if (payMethod == getTranslated(context, 'RAZORPAY_LBL')||payMethod == getTranslated(context, 'PAYUMONEY_LBL')) {
              addTransaction(tranId, orderId, SUCCESS, msg, true);
            } else if (payMethod == getTranslated(context, 'PAYPAL_LBL')) {
              paypalPayment(orderId);
            } else if (payMethod == getTranslated(context, 'STRIPE_LBL')) {
  ...
  String _paymentStatus = '';
   ...
   Future<void> initiatePayment() async {
    ...
    if(data!=null){
        http.post(Uri.parse("https://giftsbash.com/app/v1/api/check_phonepay_status"),body: {
  ...
  Future<Map <String, dynamic>> fetchPaymentStatus () async {
    ...
    print(url);
    var request = http.MultipartRequest('POST', Uri.parse('https://giftsbash.com/app/v1/api/check_phonepay_status'));
    request.fields.addAll({
      'transaction_id': '${merchantTransactionId}'
    });
  ...
  Future<void> getPhonpayURL() async {
    ...
    var request = http.MultipartRequest('POST', Uri.parse('https://giftsbash.com/app/v1/api/initiate_phone_payment'));
    request.fields.addAll({
      'user_id': '${CUR_USERID}',
      'order_id': '${orderId}',
      'amount': '${totalamount}'
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            InkWell(
                onTap: ()async{
                var result = await Navigator.push(context, MaterialPageRoute(builder: (context) => DeliverToPage()));
                print("checking result here ${result}");
                setState(() {
                  deliveryAddres = result;
                  receiverId = deliveryAddres['id'];
                });
                },
                child: Container(
                  height: 40,
                  ...
                  alignment: Alignment.center,
                    child: Text("Sender Details ",style: TextStyle(color: Colors.white,fontSize: 14,fontWeight: FontWeight.w500),),
                ),
              ) : SizedBox.shrink(),
            deliveryAddres == null || deliveryAddres == ""  ? SizedBox.shrink():
            Padding(
      ...
      child: InkWell(
        ...
        onTap: () async {
         ...
         final ResultData = await Navigator.push(
          ...
          print("checking delivery charge response here ${response.statusCode}");
          if (response.statusCode == 200) {
            totalamount = tempTotaleAmt.toString() ;
            var   FinalResult =   await response.stream.bytesToString();
            final jsonResponse = json.decode(FinalResult);
            print("New-----------${jsonResponse.toString()} and ${jsonResponse['delivery_charge'] }");
            setState((){
              dCharge = jsonResponse['delivery_charge'];
              double ress = double.parse(totalamount.toString()) + double.parse(dCharge.toString()) - promoAmt;
              print("newwwww ressssssssss ${ress}");
              print("newww dalll cahrgeee ${dCharge}");
              totalamount = ress.toString();
              // newTotalamt = totalamount;
              print("final totalll newwwww ${totalamount}");
            });
            print("final delivery charge newwwwwwww ${dCharge}");
          }
          else {
            print(response.reasonPhrase);
          }
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            ...
            promoAmt = double.parse(data["final_discount"]);
            print("promocode amttt is nowwwww ${promoAmt}");
            promocode = data["promo_code"];
            isPromoValid = true;
            setSnackbar(
                getTranslated(context, 'PROMO_SUCCESS')!, _checkscaffoldKey);
            totalamount = (double.parse(totalamount.toString()) - promoAmt).toString();
          } else {
            isPromoValid = false;
            promoAmt = 0;
            promocode = null;
