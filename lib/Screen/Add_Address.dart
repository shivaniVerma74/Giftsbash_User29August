...
class StateAddress extends State<AddAddress> with TickerProviderStateMixin {
  ...
  checkPincode()async{
    var headers = {
      'Cookie': 'ci_session=483df9e3bb917dc397680f9cce9c3e9d32e56b54'
    };
    var request = http.MultipartRequest('POST', Uri.parse('${baseUrl}check_pincode'));
    request.fields.addAll({
      'pincode': pincodeC!.text
    });
    print("parameters ${request.fields}");
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      var finalResult = await response.stream.bytesToString();
      final jsonResponse = json.decode(finalResult);
      print("final jsonResponse here ${jsonResponse}");
      if(jsonResponse['error'] == false){
        ...
        Fluttertoast.showToast(msg: "Service is not available to this pincode");
      }
    }
    else {
  ...
  Future<void> addNewAddress() async {
    ...
    try {
      ...
      print("New Adddd============${updateAddressApi.toString()}");
      print(data.toString());
